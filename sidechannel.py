...
def get_response_delay(url, data, username):
	...
	start = time.time()
	try:
		response = urllib2.urlopen(request, data % username)
	except:
		pass
...
if __name__ == "__main__":
	...
	if len(sys.argv) < 6:
		print "Usage: python %s [url] [valid_user] [path] [delay] [data]\n" % sys.argv[0]
		...
		print "\tData       Post data (%s need to be placed instead of the username)"
		sys.exit(0)
